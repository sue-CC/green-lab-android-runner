    ...
    for row in reader:
        for k, v in list(row.items()):
            if not k.split('[')[0].strip() == 'Time' and not v == '':
                column_dicts[k].append(v)
    print(column_dicts)
    return column_dicts

def aggregate_batterymanager_final(logs_dir):
    ...
    for run_file in [f for f in os.listdir(logs_dir) if os.path.isfile(os.path.join(logs_dir, f))]:
        fname = os.path.join(logs_dir, run_file)
        if not fname.endswith(".csv"):
            continue
        with open(fname, 'r') as run:
            run_dict = {}
    ...
    for device in list_subdir(data_dir):
        ...
        for subject in list_subdir(device_dir):
            ...
            if os.path.isdir(os.path.join(subject_dir, 'batterymanager')):
                row.update(aggregate_batterymanager_final(os.path.join(subject_dir, 'batterymanager')))
                rows.append(row.copy())
            else:
                for browser in list_subdir(subject_dir):
                    row.update({'browser': browser})
                    browser_dir = os.path.join(subject_dir, browser)
                    if os.path.isdir(os.path.join(browser_dir, 'batterymanager')):
                        row.update(aggregate_batterymanager_final(os.path.join(browser_dir, 'batterymanager')))
                        rows.append(row.copy())
    return rows
...
if __name__ == '__main__':
    ...
    main(None, '/home/radu/VU/GreenLab/BM-experiment/Pixel3-W/Experiment1000/batch1/output/2023.06.11_120318/data/',
         './output.csv')
